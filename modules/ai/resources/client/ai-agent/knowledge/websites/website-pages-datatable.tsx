...
const columns: ColumnConfig<AiAgentWebpage>[] = [
  {
    ...
    body: page => (
      <div>
        <Link
          to={page.fully_scanned ? `${page.id}${window.location.search}` : ''}
    ...
    body: page => (
      <div className="text-muted">
        <IconButton
          size="md"
          elementType={Link}
          to={`${page.id}${window.location.search}`}
    ...
    <DatatablePageWithHeaderLayout>
      ...
      <DatatablePageHeaderBar showSidebarToggleButton>
        <Breadcrumb size="xl">
          ...
          <BreadcrumbItem to={`../knowledge/websites${location.search}`}>
...
export function DeleteWebpagesDialog({
  selectedIds,
  onDelete,
